(ns matillion.main
  "Kotodama WASM entrypoint for the Matillion clean-room actor (L4) — Clojure port.
  ...
  facts (`matillion.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "matillion")
(def tier "L4")
...
(def entity-specs
  [{:entity "Connector" :plural "connectors" :id-prefix "matillio_con"
    :fields [:name :source :destination :status] :required [:name :source]
    :coerce {}                       :refs {}}
   {:entity "Sync"      :plural "syncs"      :id-prefix "matillio_syn"
    :fields [:connectorId :status :rows :startedAt] :required [:status :rows]
    :coerce {:rows :int}             :refs {:connectorId "Connector"}}
   {:entity "Model"     :plural "models"     :id-prefix "matillio_mod"
    :fields [:name :materialization :sql]          :required [:name :materialization]
    :coerce {}                       :refs {}}
   {:entity "Stream"    :plural "streams"    :id-prefix "matillio_str"
    :fields [:name :partitions :retentionMs]      :required [:name :partitions]
    :coerce {:partitions :int :retentionMs :int} :refs {}}
   {:entity "Schema"    :plural "schemas"    :id-prefix "matillio_sch"
    :fields [:name :database :tableCount]         :required [:name :database]
    :coerce {:tableCount :int}       :refs {}}
   {:entity "Test"      :plural "tests"      :id-prefix "matillio_tes"
    :fields [:modelName :type :passed]            :required [:modelName :type]
    :coerce {:passed :bool}          :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"matillion.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "matillion-compat" :tier tier :entities entities} 200])
...
(defn start! [] :matillion-compat/ready)
